...
/* * /
    	...
    	//This determines which directions are possible for the PacMan to travel.
    	if((lastAction == NOTHING) || (lastAction == Pac.N) || (lastAction == Pac.E) || (lastAction == Pac.S) || (lastAction == Pac.W)) 
    	{
    		//Determines if PacMan should be able to go North
    		if (isPossibleToDoAction(Pac.N) && lastAction != Pac.S)
    		{
    			possible_directions += "N";	
    		}
    			//Determines if PacMan should be able to go East
    			if (isPossibleToDoAction(Pac.E) && lastAction != Pac.W)
    			{
    				possible_directions += "E";		
    			}
    				//Determines if PacMan should be able to go West
    				if (isPossibleToDoAction(Pac.S) && lastAction != Pac.N)
    				{
    					possible_directions += "S";	
    				}
    					//Determines if PacMan should be able to go South
    					if (isPossibleToDoAction(Pac.W) && lastAction != Pac.E)
    					{
    						possible_directions += "W";
    					}
    		
    	//Chooses a possible direction.
    	Random rand = new Random();
    	int number_of_possible_directions = possible_directions.length(); 
   		char chosen_direction = possible_directions.charAt(rand.nextInt(number_of_possible_directions));

   		//If the chosen direction is North, then makes the PacMan go North.
   		if(chosen_direction == 'N')
   			{
   				pacman.actions[0] = Pac.N;
   				performAction(Pac.N);
    		}
   			//If the chosen direction is East, then makes the PacMan go East.
   			else if(chosen_direction == 'E')
   				{
   					pacman.actions[0] = Pac.E;
   					performAction(Pac.E);
   				}
   				//If the chosen direction is South, then makes the PacMan go South.
   				else if(chosen_direction == 'S')
   					{
   						pacman.actions[0] = Pac.S;
   						performAction(Pac.S);
   					}
   					//If the chosen direction is West, then makes the PacMan go West.
